...
const Topbar: FC = () => {
  ...
  const [rawPixelGridOptions = ['32x32', '32x48'], setRawPixelGridOptions] = useLocalStorage<string[]>(
    CONFIG_KEY.PixelGridOptions,
  );
  ...
  return (
    <>
      <div className={styles.topbar}>
        ...
        <Button className={styles.item} icon={<IconUndo />} disabled={!canUndo} onClick={undo} />
        <Button className={styles.item} icon={<IconRedo />} disabled={!canRedo} onClick={redo} />
      ...
      {!contextIsNormal && (
        <Banner
          ...
          description={
            <span>
              当前图片的尺寸为{width}x{height}，与网格尺寸不匹配，可能导致编辑错误，请更换图片或
